...
total_slots = 12  # Total number of parking slots
slot_status = {i: True for i in range(1, total_slots + 1)}
...
#        print(row)
    ...
    for i in range(1, total_slots + 1):
        slot_status[i] = True  # Reset all slots as occupied
    for i, lst in enumerate([list1, list2, list3, list4, list5, list6, list7, list8, list9, list10, list11, list12], start=1):
        if len(lst) == 0:
            slot_status[i] = False  # Mark slot as free if no car detected

    # Get numbers of free slots
    free_slots_numbers = [slot for slot, status in slot_status.items() if not status]

    # Display free slot numbers
    free_slots_text = " ".join(str(slot) for slot in free_slots_numbers)
    cv2.putText(frame, f"Free Slots: {free_slots_text}", (250, 50), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0,0,255), 2)
    
    cv2.putText(frame,f"Free:{str(space)}/12",(20,50),cv2.FONT_HERSHEY_COMPLEX,1,(0,0,255),2)
